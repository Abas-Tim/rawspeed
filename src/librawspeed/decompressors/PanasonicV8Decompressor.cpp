...
struct BitStreamerReversedSequentialReplenisher : public BitStreamerForwardSequentialReplenisher<Tag> {
  ...
  // Almost an exact copy of BitStreamerForwardSequentialReplenisher::getInput(), but here 
  // we flip the order of the bits within each byte, as they get loaded. 
  std::array<std::byte, BitStreamerTraits<Tag>::MaxProcessBytes> getInput() {
    ...
    auto tmp = Array1DRef<std::byte>(tmpStorage.data(), implicit_cast<int>(tmpStorage.size()));

    if (Base::getPos() + BitStreamerTraits<Tag>::MaxProcessBytes <= Base::input.size()) [[likely]] {
      auto currInput = Base::input.getCrop(Base::getPos(), BitStreamerTraits<Tag>::MaxProcessBytes).getAsArray1DRef();
      invariant(currInput.size() == tmp.size());
      std::copy_n(currInput.begin(), BitStreamerTraits<Tag>::MaxProcessBytes, tmp.begin());

      // Reverse the order of bits within each byte using a bit-twiddle trick.
      // Three operation bit reversal from: https://graphics.stanford.edu/~seander/bithacks.html#ReverseByteWith64BitsDiv
      for (std::byte& b : tmp) {
        b = std::byte{uint8_t((uint8_t(b) * 0x0202020202ULL & 0x010884422010ULL) % 1023)};
      }

      return tmpStorage;
    }

    if (Base::getPos() > Base::input.size() + 2 * BitStreamerTraits<Tag>::MaxProcessBytes) [[unlikely]]
...
class BitStreamerRevMSB final : public BitStreamer<BitStreamerRevMSB, BitStreamerReversedSequentialReplenisher<BitStreamerRevMSB>> {
  using Base = BitStreamer<BitStreamerRevMSB, BitStreamerReversedSequentialReplenisher<BitStreamerRevMSB>>;
public:
  using Base::Base;
};
...
private:
  ...
  const std::vector<uint16_t>& mShiftDownList; // Reference to PanasonicV8Decompressor mParams.huffShiftDown
  BitStreamerRevMSB mBitPump;

public:
  InternalHuffDecoder(const PanasonicV8Decompressor::HuffmanLUT& LUT, const std::vector<uint16_t>& shiftDownList, Array1DRef<const uint8_t> bitStream)
...
PanasonicV8Decompressor::PanasonicV8Decompressor(Buffer inputFile, RawImage outputImg, const TiffIFD& ifd)
: mInputFile(inputFile), mRawOutput(std::move(outputImg))
{
  if (mRawOutput->getCpp() != 1 || mRawOutput->getDataType() != RawImageType::UINT16 || mRawOutput->getBpp() != sizeof(uint16_t)) {
    ThrowRDE("Unexpected component count / data type");
  }

  mParams.horizontalStripCount = ifd.getEntry(TiffTag::PANASONIC_V8_NUMBER_OF_STRIPS_H)->getU16();
  mParams.verticalStripCount = ifd.getEntry(TiffTag::PANASONIC_V8_NUMBER_OF_STRIPS_V)->getU16();
  
  getPanasonicTiffVector(ifd, TiffTag::PANASONIC_V8_STRIP_BYTE_OFFSETS, mParams.stripByteOffsets);
  getPanasonicTiffVector(ifd, TiffTag::PANASONIC_V8_STRIP_LINE_OFFSETS, mParams.stripLineOffsets);
  getPanasonicTiffVector(ifd, TiffTag::PANASONIC_V8_STRIP_DATA_SIZE, mParams.stripBitLengths);
  getPanasonicTiffVector(ifd, TiffTag::PANASONIC_V8_STRIP_WIDTHS, mParams.stripWidths);
  getPanasonicTiffVector(ifd, TiffTag::PANASONIC_V8_STRIP_HEIGHTS, mParams.stripHeights);

  // Get decoder's initial prediction value:
  // Note, the positions of the green samples are swapped. This is intentional, the original
  // implementation did this each swap redundantly during decoding of each tile. 
  mParams.initialPrediction[0]  = ifd.getEntry(TiffTag::PANASONIC_V8_INIT_PRED_RED)->getU16();
  mParams.initialPrediction[2] = ifd.getEntry(TiffTag::PANASONIC_V8_INIT_PRED_GREEN1)->getU16();
  mParams.initialPrediction[1] = ifd.getEntry(TiffTag::PANASONIC_V8_INIT_PRED_GREEN2)->getU16();
  mParams.initialPrediction[3]  = ifd.getEntry(TiffTag::PANASONIC_V8_INIT_PRED_BLUE)->getU16();
  
  getPanasonicTiffVector(ifd, TiffTag::PANASONIC_V8_HUF_SHIFT_DOWN, mParams.huffShiftDown);
...
{
  #ifdef HAVE_OPENMP
    uint threadCount = std::min(int(mParams.horizontalStripCount), rawspeed_get_number_of_processor_cores());
...
void PanasonicV8Decompressor::decompressStrip(const uint stripIdx, InternalHuffDecoder decoder, Array2DRef<uint16_t> outBuffer) const
{
  ...
  for (uint row = 0; row < stripHeight; row += 2) {
    // Each decoded 'row' is actually two rows of pixels in the raw image because the image
    // is encoded in rows of 2x2 CFA tiles. Likewise the effective width here is 2x the strip width. 
    for (uint column = 0; column < stripWidth*2; ++column) {
      ...
      lineBuffer[column] = uint16_t(std::clamp(decodedValue, 0, int32_t(mParams.gammaClipVal)));

      if (ccIdx == 3) {
        // Completed decoding a 2x2 CFA tile. Update the predicted value to equal the decoded value. 
        std::copy_n(&lineBuffer[column-3], 4, predicted.data());
      }
    }
    // At the end of the line, reset predicted value to the first tile of the prior line. 
    ...
    for (uint linePos = 0; linePos < stripWidth*2; linePos += 4) {
      ...
      if (mGammaLUT.empty()) [[likely]] {
        ...
        outBuffer[row + 0](dstStartCol + 1) = lineBuffer[linePos + 2]; // Top Green
        outBuffer[row + 1](dstStartCol + 0) = lineBuffer[linePos + 1]; // Bottom Green
        outBuffer[row + 1](dstStartCol + 1) = lineBuffer[linePos + 3]; // Bottom Blue
      } else [[unlikely]] {
        outBuffer[row + 0](dstStartCol + 0) = mGammaLUT[lineBuffer[linePos + 0]]; // Top Red
        outBuffer[row + 0](dstStartCol + 1) = mGammaLUT[lineBuffer[linePos + 2]]; // Top Green
        outBuffer[row + 1](dstStartCol + 0) = mGammaLUT[lineBuffer[linePos + 1]]; // Bottom Green
        outBuffer[row + 1](dstStartCol + 1) = mGammaLUT[lineBuffer[linePos + 3]]; // Bottom Blue
      }
    }
    // TODO: Investigate if it makes sense performance wise to structure lineBuffer such that it can be
    // memcpy'd into the output Buffer.
  }
}
...
int32_t inline PanasonicV8Decompressor::InternalHuffDecoder::decodeNextDiffValue() {
  ...
  if (diffBitCount > 0) {
    // Decode difference value. The scheme here encodes signed integers in a manner
    // similar to offset binary encoding. Here, the encoding is biased by the difference
    // category such that abs(diff) is in the range [2^{diffCat-1}, 2^{diffCat}). 
  ...
  // diffBitCount of zero indicates no difference (next pixel is same as predicted)
  return 0;
}

void PanasonicV8Decompressor::validateParams() {
  if (mParams.verticalStripCount != 1)
    ThrowRDE("RW2v8 file contains %d vertical strips. This is unsupported", mParams.verticalStripCount);
  ...
  if (mParams.horizontalStripCount > mParams.stripByteOffsets.size())
    ThrowRDE("Strip byte offset list does not have enough entries for the number of strips!");
  if (mParams.horizontalStripCount > mParams.stripWidths.size())
    ThrowRDE("Strip widths list does not have enough entries for the number of strips!");
  if (mParams.horizontalStripCount > mParams.stripHeights.size())
    ThrowRDE("Strip heights list does not have enough entries for the number of strips!");
  if (mParams.horizontalStripCount > mParams.stripLineOffsets.size())
    ThrowRDE("Strip line offset list does not have enough entries for the number of strips!");
  if (mParams.horizontalStripCount > mParams.stripBitLengths.size())
    ThrowRDE("Strip bit length list does not have enough entries for the number of strips!");

  if (std::any_of(mParams.huffShiftDown.begin(), mParams.huffShiftDown.end(), [](uint16_t x){ return x != 0; })) {
    ThrowRDE("Non-zero shift down value encountered! Shift down decoding has never been tested!");
  }
}
...
void PanasonicV8Decompressor::populateGammaLUT(const TiffIFD& ifd) {
  ...
  getPanasonicTiffVector(ifd, TiffTag::PANASONIC_V8_GAMMA_POINTS, encodedGammaPoints);
  getPanasonicTiffVector(ifd, TiffTag::PANASONIC_V8_GAMMA_SLOPES, encodedGammaSlopes);

  // Determine if the points and slopes are all set to zero and 65536 respectively. If so, no gamma 
  // function needs to be applied. This is currently true of all tested RW2 files. 
  const bool gamamPointsAreIdentity = std::all_of(
    encodedGammaPoints.cbegin(), encodedGammaPoints.cend(),
    [](const uint32_t p) { return p == 0u; });
  const bool gammaSlopesAreIdentity = std::all_of(
   encodedGammaSlopes.cbegin(),encodedGammaSlopes.cend(),
  ...
  if (!gamamPointsAreIdentity || !gammaSlopesAreIdentity) {
    ...
    // Decode point and slope lists. Defines a piece-wise function for the gamma curve.
    // Each point is an x,y intercept of a line segment with slope encoded with a power
    // of two exponent and sign bit. The x position of the points must be ordered such that
    // together they create six non-overlapping intervals covering [0, UINT16_MAX]. 
    struct GammaPoint { uint16_t x, y; };
    struct GammaSlope { uint8_t sign, exp; };
    std::array<GammaPoint, 6> gammaPoints;
    std::array<GammaSlope, 6> gammaSlopes;
    for (uint i = 0; i < 6; ++i) {
      gammaPoints[i] = {uint16_t(encodedGammaPoints[i] & 0xFFFF), uint16_t(encodedGammaPoints[i] >> 16)};
    ...
    const bool pointsAreOrdered = std::is_sorted(gammaPoints.cbegin(), gammaPoints.cend(), 
    ...
    const auto fnGamma = [&](const uint32_t x, const uint i) -> uint16_t {
      ...
      if (slope.exp == 15 && slope.sign == 1) {
        // An exponent of 15 and sign of 1 signals a special case where mx becomes the 
        // y-intercept of the next curve segment or UINT16_MAX if there is no next segment. 
        ...
        // Negative slope 
        uint32_t h = 1 << (slope.exp - 1); // Add slope/2 so that integer arithmetic for `mx` will round correctly. 
    ...
    for (uint32_t x = 0; x <= UINT16_MAX; ++x) {
      ...
      assert(interval+1 == gammaPoints.size() || gammaPoints[interval+1].x > x);
