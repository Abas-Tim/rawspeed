...
struct DemosaicJob {
  ...
  uint32_t rgbWidth = 0;
  uint32_t rgbHeight = 0;
  int patternCode = 0;
  std::array<float, 3> wb;

  DemosaicJob(rawspeed::Array2DRef<uint16_t> m,
              rawspeed::Array1DRef<uint16_t> r)
      : mosaic(m), rgb(r), wb{1.0F, 1.0F, 1.0F} {}
};

int cfaIndexOf(int patternCode, uint32_t row, uint32_t col) {
...
void demosaicBand(const DemosaicJob& j, uint32_t yStart, uint32_t yEnd) {
  static constexpr std::array<float, 5> weightByDist = {4.0F, 1.0F, 0.25F,
                                                        0.25F, 0.25F};
    ...
    for (uint32_t x = 0; x < j.rgbWidth; ++x) {
      ...
      for (int dy = -2; dy <= 2; ++dy) {
        ...
        for (int dx = -2; dx <= 2; ++dx) {
          ...
          const auto v = static_cast<float>(j.mosaic(yy, xx));
          const int dist = std::abs(dx) + std::abs(dy);
          const auto w = weightByDist.at(dist);
          acc.at(c) += v * w;
          counts.at(c)++;
        }
      }
      ...
      const auto pix =
          static_cast<int>(static_cast<size_t>(y) * j.rgbWidth + x);
...
bool render(const rawspeed::RawImage& raw, std::vector<uint16_t>& rgbOut,
  ...
  const auto w = static_cast<uint32_t>(mosaic.width());
  const auto h = static_cast<uint32_t>(mosaic.height());
  if (w == 0 || h == 0) {
